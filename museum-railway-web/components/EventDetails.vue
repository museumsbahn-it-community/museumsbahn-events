  ...
  <card>
    <template #content>
      <div v-if="event != undefined" class="flex flex-column gap-3">
        <EventImage :event="event" v-if="viewport.isLessThan('tablet')"
                    class="flex justify-content-end max-dimensions"/>
        <div class="p-3 gap-3 flex flex-column">
          <div class="flex flex-row gap-3 event-details-header">
            ...
            <EventImage v-if="!viewport.isLessThan('tablet')" :event="event"
